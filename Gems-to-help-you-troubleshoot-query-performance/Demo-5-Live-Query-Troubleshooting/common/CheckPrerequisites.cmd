...
REM Check for AdventureWorks2016CTP3 database
sqlcmd.exe -E -S%SQLSERVER% -Q"exit (select count(*) from master..sysdatabases where name = 'AdventureWorks2016CTP3')" > NUL 2>&1
...
:InstallAW
@ECHO %time% - Attempting to install AdventureWorks2016CTP3 database (this is a one-time operation)
@ECHO %time% - Extracting AdventureWorks2016CTP3 database backup...
..\bin\7za.exe e -y awdb.zip -o%TEMP%
IF %errorlevel% neq 0 (
  @ECHO ERROR: Unzip failed
  GOTO :ManualAWInstructions
)
@ECHO %time% - Restoring AdventureWorks2016CTP3 database...
sqlcmd -E -S%SQLSERVER% -Q"restore database AdventureWorks2016CTP3 from disk = '%temp%\awdb.bak' with stats=10"
del %TEMP%\awdb.bak
sqlcmd.exe -E -S%SQLSERVER% -Q"exit (select count(*) from master..sysdatabases where name = 'AdventureWorks2016CTP3')"  > NUL
IF %errorlevel%==0 (
  @ECHO.
